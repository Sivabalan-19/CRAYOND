...
import cartt from '../component/assets/cart.png';
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            <div className="searchandicon">
                <div style={{ width: "85%", padding: "15px", height: "70%", display: "flex", alignItems: "center" }}>
                    <Searchbar />
                </div>
                <button className="icononlyboxbok" style={styles(true)} onClick={() => setcol(-2)}>
            ...
            <div className="threeiconandlist">
                <div style={{ width: "28%", height: "100%", alignItems: "center", justifyContent: "start", display: "flex", flexDirection: "column" }}>
                    <div style={{ fontSize: "22px", justifyContent: "start", display: "flex" }}>
                        Cart Summary
                    </div>
                    <div style={{ fontSize: "17px", justifyContent: "start", display: "flex" }}>
                        Order Id: 000007
                    </div>
                </div>
                <div style={{ width: "30%", alignItems: "center", height: "100%", display: "flex", padding: "0% 3%", justifyContent: "space-between" }}>
                ...
                </div>
                {
                    cart.length === 0 ? (
                        <div className="emptypage">
                            <div>
                                <div style={{ width: '100%', textAlign: 'center' }}><img src={cartt} alt="" style={{ height: '50%', width: '50%' }} /></div>
                                <div style={{ width: '100%', textAlign: 'center', fontWeight: '700', fontSize: '18px' }}>Cart is Empty</div>
                        ...
                        <div className='dish-pricelis'>
                            {
                                cart.map((item, index) => (
                                    <div className='trbox' key={index}>
                                        <div className="tablecontvalue" style={{ width: '45%', fontSize: "20px", fontWeight: '600', justifyContent: 'start' }}>{item.name}</div>
                                        <div className="tablecontvalue" style={{ width: '25%', textAlign: 'center' }}>
                                            <div className="addsubbot" style={{ width: '60%' }}>
                                                <button onClick={() => handleSubtract(index)}><LuMinus /></button>
                                                <button>{item.quantity}</button>
                                                <button onClick={() => handleAdd(index)}><IoAdd /></button>
                                            </div>
                                        </div>
                                        <div className="tablecontvalue" style={{ width: '25%', fontSize: '18px', textAlign: 'center', fontWeight: '700' }}>
